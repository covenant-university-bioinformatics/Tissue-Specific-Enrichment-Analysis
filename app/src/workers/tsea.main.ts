...
import { WorkerJob } from '../jobqueue/queue/tsea.queue';
import { Job, QueueScheduler, Worker } from 'bullmq';
import { JobStatus, TseaJobsModel } from '../jobs/models/tsea.jobs.model';
import * as path from 'path';
import { TseaModel } from '../jobs/models/tsea.model';
...
const processorFile = path.join(__dirname, 'tsea.worker.js');
...
export const createWorkers = async (
  ...
  for (let i = 0; i < config.numWorkers; i++) {
    ...
    worker.on('completed', async (job: Job, returnvalue: any) => {
      ...
      const resultsFile = `${pathToOutputDir}/Tissue_Specific_Enrichment.txt`;

      //update db with result files
      const finishedJob = await TseaJobsModel.findByIdAndUpdate(
        job.data.jobId,
        {
          status: JobStatus.COMPLETED,
          tseaResultsFile: resultsFile,
